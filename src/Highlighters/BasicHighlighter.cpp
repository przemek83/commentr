...
void BasicHighlighter::initRules()
{
    ...
    keywordPatterns
        << "\\bAddHandler\\b" << "\\bAddressOf\\b" << "\\bAlias\\b"
        ...
        << "\\bDecimal\\b" << "\\bDeclare\\b" << "\\bDefault\\b"
        << "\\bDelegate\\b" << "\\bDim\\b" << "\\bDirectCast\\b" << "\\bDo\\b"
        << "\\bDouble\\b" << "\\bEach\\b" << "\\bElse\\b" << "\\bElseIf\\b"
        << "\\bEnd\\b" << "\\bEndIf\\b" << "\\bEnum\\b" << "\\bErase\\b"
        << "\\bError\\b" << "\\bEvent\\b" << "\\bExit\\b" << "\\bFalse\\b"
        << "\\bFinally\\b" << "\\bFor\\b" << "\\bFriend\\b" << "\\bFunction\\b"
        << "\\bGet\\b" << "\\bGetType\\b" << "\\bGetXMLNamespace\\b"
        << "\\bGlobal\\b" << "\\bGoSub\\b" << "\\bGoTo\\b" << "\\bHandles\\b"
        << "\\bIf\\b" << "\\bImplements\\b" << "\\bImports\\b"
        << "\\bImports\\b" << "\\bIn\\b" << "\\bInherits\\b" << "\\bInteger\\b"
        << "\\bInterface\\b" << "\\bIs\\b" << "\\bIsNot\\b" << "\\bLet\\b"
        << "\\bLib\\b" << "\\bLike\\b" << "\\bLong\\b" << "\\bLoop\\b"
        << "\\bMe\\b" << "\\bMod\\b" << "\\bModule\\b" << "\\bMustInherit\\b"
        << "\\bMustOverride\\b" << "\\bMyBase\\b" << "\\bMyClass\\b"
        << "\\bNamespace\\b" << "\\bNarrowing\\b" << "\\bNew\\b" << "\\bNext\\b"
        << "\\bNot\\b" << "\\bNothing\\b" << "\\bNotInheritable\\b"
        ...
        << "\\bPrivate\\b" << "\\bProperty\\b" << "\\bProtected\\b"
        << "\\bPublic\\b" << "\\bRaiseEvent\\b" << "\\bReadOnly\\b"
        << "\\bReDim\\b" << "\\bREM\\b" << "\\bRemoveHandler\\b"
        << "\\bResume\\b" << "\\bReturn\\b" << "\\bSByte\\b" << "\\bSelect\\b"
        << "\\bSet\\b" << "\\bShadows\\b" << "\\bShared\\b" << "\\bShort\\b"
        << "\\bSingle\\b" << "\\bStatic\\b" << "\\bStep\\b" << "\\bStop\\b"
        << "\\bString\\b" << "\\bStructure\\b" << "\\bSub\\b"
        ...
        << "\\bWidening\\b" << "\\bWith\\b" << "\\bWithEvents\\b"
        << "\\bWriteOnly\\b" << "\\bXor\\b" << "(^|\\s)#Const\\b"
        << "(^|\\s)#Else\\b" << "(^|\\s)#ElseIf\\b" << "(^|\\s)#End\\b"
        << "(^|\\s)#If\\b";
    ...
    rule.startPattern_ = QRegularExpression("\".*\"");
